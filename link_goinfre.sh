...
get_case(){
    ...
    if [[ -L "$source_dir" && -d $dest_dir ]]; then
      echo "Link - Folder"
      return $LINK_FOLDER
    elif [[ -L "$source_dir" && -L $dest_dir ]]; then
      echo "Link - Link"
      return $LINK_LINK
    ...
    elif [[ -d "$source_dir" && -L $dest_dir ]]; then
      echo "Folder - Link"
      return FOLDER_LINK
      ...
      echo "None - Folder"
      return $NONE_FOLDER
    elif [[ ! -e "$source_dir" && -L $dest_dir ]]; then
      echo "None - Link"
      return $NONE_LINK
    elif [[ ! -e "$source_dir" && ! -e $dest_dir ]]; then
      echo "None - None"
      return $NONE_NONE
    fi
...
#    move_and_link "$path" "$destination"
    ...
    let RETURN_VALUE=$?
    echo ""$path" "$destination" RETURN_VALUE: $RETURN_VALUE"
