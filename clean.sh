...
find ~/Library/Application\ Support/Slack/Cache -type f -name "*" -exec rm {} \;
find ~/Library/Application\ Support/Slack/Service\ Worker/CacheStorage/ -type f -name "*" -exec rm {} \;
find ~/Library -type d -name "Cache" -exec rm -r {} \;

rm -r ~/Library/*/*.keepcoder.Telegram/stable/account-*/postbox/media
find ~/Library/Caches/com.apple.* -print0 | xargs -0 -I{.} find {.} -type f -print0 | xargs -0 -I{.} rm {.}
...
rm -rf ~/Library/Application\ Support/Slack/Service\ Worker/CacheStorage
rm -rf ~/Library/Application\ Support/Spotify/PersistentCache
rm -rf ~/Library/Group\ Containers/*.ru.keepcoder.Telegram/stable/account-*/postbox/media/telegram-cloud-*
echo "clean complete"
free_space=$(df -h | grep 'evangelm' | awk '{print $4}')
